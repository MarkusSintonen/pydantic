...
class _Definitions:
    ...
    def finalize_schema_defs_inplace(self, schema: core_schema.CoreSchema) -> core_schema.CoreSchema:
        ...
        try:
            ...
            def_refs_by_name, recursive_refs = gather_res['definition_refs'], gather_res['recursive_refs']
        except GatherInvalidDefinitionError as e:
            raise PydanticUndefinedAnnotation(name=str(e), message='undefined ref')

        if (schema_ref := schema.get('ref')) is not None and schema_ref in recursive_refs:
        ...
        for ref, def_refs in def_refs_by_name.items():
            ...
            if inline_def_ref is not None and self._can_be_inlined(ref, inline_def_ref, recursive_refs):
                ...
                inline_def_ref.update(self._definitions[ref])  # type: ignore
            else:
                remaining_defs[ref] = self._definitions[ref]

        if self.used_deferred_discriminators:
            found_schemas = (gather_res.get('schemas_with_meta_keys') or {}).get(discriminator_meta_key)
            for apply_to_schema in found_schemas or []:
                if (discriminator := apply_to_schema.get('metadata', {}).get(discriminator_meta_key)) is not None:
